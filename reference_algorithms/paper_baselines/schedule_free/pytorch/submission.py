...
def init_optimizer_state(
  ...
  optimizer_state = {
    'optimizer': optimizer,
    'max_checked_eval_step': -1,
    'has_forced_reset': False,
    'first_eval': False,
  }
...
def update_params(
  ...
  if (global_step > workload.step_hint * 0.10) and optimizer_state[
    'max_checked_eval_step'
  ] < eval_step:
    optimizer_state['max_checked_eval_step'] = eval_step
    # Don't do resetting on workloads that don't run eval often enough
    if (
      len(eval_results) >= 4
    ):  # and optimizer_state["first_eval_is_far_from_target"]:
      ...
      if (
        (latest_eval - initial_eval < MARGIN)
        and (latest_eval - second_latest_eval < MARGIN)
        and (second_latest_eval - third_latest_eval < MARGIN)
        and (third_latest_eval - fourth_latest_eval < MARGIN)
      ):
