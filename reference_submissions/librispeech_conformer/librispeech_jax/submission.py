...
def pmapped_train_step(workload, opt_update_fn, model_state, optimizer_state,
  ...
  def _loss_fn(params):
    ...
    (logits, logit_paddings), new_model_state = workload.model_fn(
        params, batch, model_state, spec.ForwardPassMode.TRAIN, {
            'params': params_rng,
            'dropout': dropout_rng
        })

    loss = workload.loss_fn(logits, logit_paddings, batch['targets'],
...
def log_pytree_shape_and_statistics(pytree, writer):
  ...
  writer.text('model', shape_dict.pretty_repr(), 0)

  total_params = jax.tree_util.tree_reduce(
      operator.add, jax.tree_map(lambda x: x.size, pytree))
  logging.info('num params = %d', total_params)
  writer.text('num_params', total_params, 0)
...
def update_params(
  ...
  if global_step <= 1000 or global_step % 100 == 0:
    logging.info('%d) loss = %0.3f, grad_norm = %0.3f lr = %0.6f', global_step,
                 loss.mean(), grad_norm.mean(), lr)
    workload.summary_writer.scalar('train_step_ctc_loss', loss.mean(),
                                   global_step)
