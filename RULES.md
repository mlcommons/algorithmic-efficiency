...
) -> (logits_output_batch, new_model_state): Tuple[Tensor, spec.ModelAuxiliaryState]
```
...
The submission functions are the **batch size getter**, **optimizer state initializer**, **variable update**, and **data selection functions**.
...
- Does not involve the initialization for the model parameters, which in the training algorithm track, is considered a fixed function.
...
update_params(
    ...
    model_state: ModelAuxiliaryState,
    hyperparameters: Hyperparamters,
    augmented_and_preprocessed_input_batch: Dict[Tensor],
...
- r(p, s)  =  Time spent on problem p by submission s / Time spent on problem p by best submission
  ...
  - Can take on values between [1, infinity), lower is better.

<p align="center">
  <img src="https://render.githubusercontent.com/render/math?math=r_{p,s}=\frac{t_{p,s}}{\min\{t_{p,s}:s \in \mathcal{S}\}}">
</p>
...
<p align="center">
  <img src="https://render.githubusercontent.com/render/math?math=\rho_s(\tau)=\frac{1}{n_p}\size\{p \in \mathcal{P}: r_{p,s} \leq \tau\}">
</p>

For a given problem, we define the “speedup over the reference” as t_{p, ref}/t_{p,s}. For example, if a submission was 2x faster than the reference implementation, this would be equal to 2.

To have a simpler to interpret number for press releases, we will also release (in addition to the raw t_{p,s} values) the geometric mean of t_{p, ref}/t_{p,s}
