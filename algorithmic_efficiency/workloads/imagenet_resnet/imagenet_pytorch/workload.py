...
class ImagenetResNetWorkload(BaseImagenetResNetWorkload):
    ...
    def _build_dataset(
        ...
        dataset = ImageFolder(
            os.path.join(data_dir, folder[split]), transform=transform_config[split]
        )
        ...
        dataloader = data_utils.PrefetchedWrapper(
            dataloader, DEVICE, self.train_mean, self.train_stddev
        )
