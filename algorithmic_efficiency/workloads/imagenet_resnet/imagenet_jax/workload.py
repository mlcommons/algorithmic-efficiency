...
from algorithmic_efficiency.workloads.imagenet_resnet.imagenet_jax import input_pipeline
...
class ImagenetResNetWorkload(BaseImagenetResNetWorkload):
    ...
    def initialized(self, key, model):
        ...
        variables = jax.jit(model.init)(
            {"params": key}, jnp.ones(input_shape, model.dtype)
        )
    ...
    def output_activation_fn(
        self, logits_batch: spec.Tensor, loss_type: spec.LossType
    ) -> spec.Tensor:
    ...
    # `update_params`.
    def loss_fn(
        self, label_batch: spec.Tensor, logits_batch: spec.Tensor
    ...
    def _eval_model_on_split(
        ...
        eval_metrics = jax.tree_map(lambda x: float(x[0] / num_examples), eval_metrics)
        return eval_metrics
