...
def make_causal_mask(
    x: Tensor, device: str = "cuda:0", dtype: torch.dtype = torch.float32
) -> Tensor:
...
def make_tgt_and_memory_mask(
    ...
    if inputs_segmentation is not None:
        tgt_mask = torch.logical_and(
            tgt_mask,
            make_attention_mask(targets_segmentation, targets_segmentation, torch.eq),
        )
        memory_mask = torch.logical_and(
            memory_mask,
            make_attention_mask(targets_segmentation, inputs_segmentation, torch.eq),
        )
    # Flip values and ensure numerical stability.
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = Encoder(d_model, nhead, d_hid, nlayers, dropout, layer_norm_eps)
        self.decoder = Decoder(d_model, nhead, d_hid, nlayers, dropout, layer_norm_eps)
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.decoder = TransformerDecoder(
            d_model, nhead, d_hid, dropout, layer_norm_eps, nlayers
        )
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, d_model, nhead, d_hid, dropout, layer_norm_eps, num_layers):
        super().__init__()
...
def multi_head_attention_forward(
    ...
    if decode:
        if cache is not None:
            ...
            attn_mask = (
                torch.arange(max_length, device=k.device) >= cache_index
