...
class CrossEntropyLoss(torch.nn.CrossEntropyLoss):
    def forward(
        self, input, target, label_smoothing=0.1
...
class WmtWorkload(BaseWmtWorkload):
    ...
    def compute_weighted_cross_entropy(
        self, logits, targets, weights=None, label_smoothing=0.1
    ):
    ...
    def predict_step(self, inputs, params, eos_id, max_decode_len, beam_size=4):
        ...
        params = (
            params.module
            if isinstance(params, (torch.nn.DataParallel, DDP))
            else params
        )
        params.eval()
            ...
            if RANK == 0:
                ...
                for key, value in batch.items():
                    ...
                    batch[key] = (
                        tensor[0]
                        if USE_PYTORCH_DDP
                        else tensor.view(-1, value.shape[-1])
                    )
                ...
                if split != "train":
                    ...
                    dist.broadcast(per_device_batch_size, src=0)
                tensor = torch.empty(
                    (n_inputs, N_GPUS, per_device_batch_size, 256),
                    dtype=torch.int64,
                    device=DEVICE,
                )
