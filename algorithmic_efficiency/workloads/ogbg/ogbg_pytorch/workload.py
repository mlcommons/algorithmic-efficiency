...
class OgbgWorkload(BaseOgbgWorkload):
    def build_input_queue(
        ...
        if RANK == 0:
            ...
            dataset_iter = super().build_input_queue(
    ...
    def _binary_cross_entropy_with_mask(
        self, labels: torch.Tensor, logits: torch.Tensor, mask: torch.Tensor
    ) -> torch.Tensor:
        """Binary cross entropy loss for logits, with masked elements."""
        if not (
            logits.shape == labels.shape == mask.shape
        ...
        return relu_logits - (logits * labels) + (torch.log(1 + torch.exp(-abs_logits)))
