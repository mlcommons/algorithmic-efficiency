...
def _process_example(kspace, kspace_shape, target, target_shape, volume_max, seed):
    ...
    num_low_frequencies = tf.cast(num_cols_float * center_fraction, dtype=tf.int32)
    ...
    scaling_norm = tf.cast(
        tf.math.sqrt(tf.cast(tf.math.reduce_prod(tf.shape(kspace)[-2:]), "float32")),
        kspace.dtype,
    )
...
def load_fastmri_split(
    ...
    h5_paths = [os.path.join(data_dir, path) for path in listdir(data_dir)][start:end]
