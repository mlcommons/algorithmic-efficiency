...
from algorithmic_efficiency.workloads.fastmri.workload import BaseFastMRIWorkload
...
class FastMRIWorkload(BaseFastMRIWorkload):
    ...
    def _build_dataset(
        self, data_rng: spec.RandomState, split: str, data_dir: str, batch_size: int
    ):
        del data_rng
    ...
    # `update_params`.
    def loss_fn(
        self, targets_batch: spec.Tensor, outputs_batch: spec.Tensor
    ) -> spec.Tensor:  # differentiable
        return F.l1_loss(outputs_batch, targets_batch, reduction="none").mean(
            dim=(1, 2)
        )
    ...
    def _eval_model_on_split(
        ...
        for _ in range(num_batches):
            ...
            batch_metrics = self._eval_metric(
                outputs,
                batch["targets"],
                batch["mean"],
                batch["std"],
