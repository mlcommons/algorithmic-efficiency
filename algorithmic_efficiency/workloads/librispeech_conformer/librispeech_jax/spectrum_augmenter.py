...
class SpecAug(nn.Module):
    ...
    def _get_mask(
        ...
        masked_portion = jax.random.uniform(
            key=global_seed, shape=(batch_size, multiplicity), minval=0.0, maxval=1.0
        )
        masked_frame_size = jnp.einsum("b,bm->bm", max_length, masked_portion).astype(
            jnp.int32
        )
        ...
        if masks_per_frame > 0:
            ...
            multiplicity_weights = (multiplicity_weights < multiplicity_tensor).astype(
                jnp.int32
            )
