"""Librispeech Conformer JAX Workload."""
import functools
import itertools
import math
...
class LibriSpeechConformerWorkload(workload.BaseLibrispeechWorkload):
  ...
  def eval_step_pmapped(
    ...
    logits, logit_paddings = self.model_fn(params, batch, model_state,
                                           spec.ForwardPassMode.EVAL)

    decoded, decoded_paddings = self.greedy_decode(logits, logit_paddings)
    normalized_loss = self.loss_fn(logits, logit_paddings, batch['targets'],
  ...
  def _eval_model_on_split(self, split: str, num_examples: int,
    ...
    logging.info('split = %s, num_examples = %d, num_batches = %d', split,
                 num_examples, num_batches)

    if split not in self._eval_iters:
      self._eval_iters[split] = itertools.cycle(
          self.build_input_queue(rng, split, data_dir, global_batch_size,
                                 num_batches))

    metrics_report = None
    for _ in range(num_batches):
      eval_batch = next(self._eval_iters[split])
      computed_metrics = self.eval_step_pmapped(params, eval_batch, model_state,
                                                rng).unreplicate()
    ...
    self.summary_writer.scalar('{}/WER'.format(split), computed_metrics['wer'],
                               global_step)
    self.summary_writer.scalar('{}/ctc_loss'.format(split),
                               computed_metrics['ctc_loss'], global_step)
