...
class LibriSpeechConformerWorkload(workload.BaseLibrispeechWorkload):
    def init_model_fn(self, rng: spec.RandomState) -> spec.ModelInitState:
        ...
        self._param_shapes = jax.tree_map(lambda x: spec.ShapeTuple(x.shape), params)
    ...
    def loss_fn(
        self, logits, logit_paddings, targets, target_paddings
    ):  # differentiable
        logprobs = nn.log_softmax(logits)
        per_seq_loss = self.ctc_loss(logprobs, logit_paddings, targets, target_paddings)
    ...
    def ctc_loss(self, logits, logit_paddings, labels, label_paddings, blank_id=0):
        return optax.ctc_loss(logits, logit_paddings, labels, label_paddings, blank_id)
    ...
    ) -> Tuple[spec.Tensor, spec.ModelAuxiliaryState]:  # pylint: disable=line-too-long
        logits, logit_paddings = self.model_fn(
            params, batch, model_state, spec.ForwardPassMode.EVAL
        )

        decoded, decoded_paddings = self.greedy_decode(logits, logit_paddings)
        normalized_loss = self.loss_fn(
            logits, logit_paddings, batch["targets"], batch["target_paddings"]
        )
    ...
    def _eval_model_on_split(
        ...
        logging.info(
            "split = %s, num_examples = %d, num_batches = %d",
            split,
            num_examples,
            num_batches,
        )
        ...
        if split not in self._eval_iters:
            self._eval_iters[split] = itertools.cycle(
                self.build_input_queue(
                    rng, split, data_dir, global_batch_size, num_batches
                )
            )
        ...
        self.summary_writer.scalar(
            "{}/WER".format(split), computed_metrics["wer"], global_step
        )
        self.summary_writer.scalar(
            "{}/ctc_loss".format(split), computed_metrics["ctc_loss"], global_step
        )
