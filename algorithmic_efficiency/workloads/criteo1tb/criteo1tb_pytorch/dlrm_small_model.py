...
class DlrmSmall(nn.Module):
  ...
  def forward(self, x, train):
    ...
    vocab_sizes = torch.asarray(self.vocab_sizes,
                                dtype=torch.int32,
                                device=self.device)

    idx_offsets = torch.asarray([0] +
                                list(torch.cumsum(vocab_sizes[:-1], dim=0)),
                                dtype=torch.int32,
                                device=self.device)

    idx_offsets = torch.tile(torch.reshape(idx_offsets, [1, -1]),
                             [batch_size, 1])
    ...
    #-1)
    scale = torch.unsqueeze(torch.repeat_interleave(
        scale, vocab_sizes, output_size=self.total_vocab_sizes),
                            dim=-1)

    self.embedding_table = nn.Embedding(self.total_vocab_sizes,
                                        self.embed_dim,
                                        device=self.device)
