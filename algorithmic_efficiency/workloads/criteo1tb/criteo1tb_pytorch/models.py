...
class DLRMResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    bot_mlp_input = self.bot_layers[0](bot_mlp_input)

    for layer in self.bot_layers[1:]:
      bot_mlp_input += layer(bot_mlp_input)
    ...
    top_mlp_input = self.top_layers[0](top_mlp_input)
    for layer in self.top_layers[1:]:
      top_mlp_input += layer(top_mlp_input)
    return top_mlp_input
...
class DlrmSmall(nn.Module):
  ...
  def forward(self, x):
    ...
    embed_features = torch.reshape(embed_features,
                                   [batch_size, -1, self.embed_dim])
    if self.embed_ln:
      embed_features = self.embed_ln(embed_features)
