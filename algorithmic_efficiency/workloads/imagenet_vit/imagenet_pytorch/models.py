...
def posemb_sincos_2d(patches, temperature=10_000.0):
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, width: int, num_heads: int = 8, dropout: float = 0.0) -> None:
        super().__init__()
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        if self.rep_size:
            rep_size = (
                self.width if self.rep_size is True else self.rep_size
