...
from algorithmic_efficiency.workloads.librispeech.librispeech_pytorch import models
...
class LibriSpeechWorkload(spec.Workload):
    ...
    def build_input_queue(
        self, data_rng, split: str, data_dir: str, global_batch_size: int
    ):
        ...
        if split == "eval_train":
            ds, _ = data_utils.random_split(
                ds,
                [self.num_eval_train_examples, len(ds) - self.num_eval_train_examples],
    ...
    def loss_fn(
        self, batch: spec.Tensor, logits_batch: spec.Tensor  # transcripts
    ) -> spec.Tensor:
    ...
    def _eval_model_on_split(
        ...
        if split not in self._eval_iters:
            data_loader = self.build_input_queue(
                rng, split, data_dir, global_batch_size
            )
