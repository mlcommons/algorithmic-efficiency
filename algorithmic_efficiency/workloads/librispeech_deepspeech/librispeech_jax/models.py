...
import functools
from typing import Any, List, Mapping, Optional, Sequence, Tuple, Type, Union, Dict
...
from algorithmic_efficiency.workloads.librispeech_conformer.librispeech_jax import \
    spectrum_augmenter
# from algorithmic_efficiency.workloads.librispeech_deepspeech.librispeech_jax import cudnn_lstm
...
class LSTMRef(nn.Module):
  ...
  def setup(self):
    num_directions = 2 if self.bidirectional else 1
    self.num_pseudo_layers = self.num_layers * num_directions

    self.W_ih = self.param('W_ih', nn.initializers.lecun_normal(), (self.num_pseudo_layers, 4*self.hidden_size, self.input_size))
    self.W_hh = self.param('W_hh', nn.initializers.orthogonal(), (self.num_pseudo_layers, 4*self.hidden_size, self.hidden_size))
    self.b_hh = self.param('b_hh', nn.initializers.zeros, (self.num_pseudo_layers, 4*self.hidden_size,))

  def paddings_to_lengths(self, paddings):
    return jnp.sum(1 - paddings, axis=-1).astype(jnp.int32)

  @nn.compact
  def __call__(self,  inputs, input_paddings=None):
    seq_lengths = self.paddings_to_lengths(input_paddings)
    batch_size = inputs.shape[0]

    h_0 = jnp.zeros((self.num_pseudo_layers, batch_size, self.hidden_size), dtype=jnp.float32)
    c_0 = jnp.zeros((self.num_pseudo_layers, batch_size, self.hidden_size), dtype=jnp.float32)
    
    return lstm_ref(inputs, h_0, c_0, self.W_ih, self.W_hh, self.b_hh, seq_lengths, self.input_size, self.hidden_size, self.num_layers, 0.0, self.bidirectional)
...
def lstm_ref(x: Array, h_0: Array, c_0: Array, W_ih: Dict[int, Array],
  ...
  def _extract_output(seq_lengths, out):
  ...
  def _select_last_carry(carry_seq, seq_lengths):
    return carry_seq[seq_lengths - 1, jnp.arange(carry_seq.shape[1])]

  def _flip_sequence(sequences, seq_lengths):
    ...
    return jax.vmap(functools.partial(jnp.roll, axis=0), in_axes=(1, 0),
        out_axes=1)(sequences, roll_amounts)[::-1]

  def lstm_cell(carry, x, *, W_ih, W_hh, b_hh):
    h, c = carry
    ...
    i = jax.nn.sigmoid(x @ W_ii.T + h @ W_hi.T + b_hi[None])
    f = jax.nn.sigmoid(x @ W_if.T  + h @ W_hf.T + b_hf[None])
    g = jax.nn.tanh(x @ W_ig.T + h @ W_hg.T + b_hg[None])
    o = jax.nn.sigmoid(x @ W_io.T + h @ W_ho.T + b_ho[None])
    c = f * c + i * g
    h = o * jax.nn.tanh(c)
    return (h, c), h
  ...
  if not bidirectional:
    ...
    for l in range(num_layers):
      cell = functools.partial(
          lstm_cell, W_ih=W_ih[l], W_hh=W_hh[l], b_hh=b_hh[l])
      cell_fn = functools.partial(scan_fn, cell)
...
class BatchRNN(nn.Module):
  ...
  def __call__(self, inputs, input_paddings, train):
    ...
    #   features=config.encoder_dim // 2 if config.bidirectional else config.encoder_dim,
