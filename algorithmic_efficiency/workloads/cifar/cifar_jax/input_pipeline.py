...
def preprocess_for_train(
    ...
    image = _random_crop(image, crop_rng, image_size, aspect_ratio_range, area_range)
...
def preprocess_for_eval(
    image, mean_rgb, stddev_rgb, dtype=tf.float32, image_size=IMAGE_SIZE
):
...
def create_split(
    ...
    def preprocess_example(example):
        ...
        if train:
            ...
            image = preprocess_for_eval(
                example["image"], mean_rgb, stddev_rgb, dtype, image_size
            )
    ...
    ds = ds.enumerate().map(
        lambda i, ex: preprocess_example((i, ex)),
