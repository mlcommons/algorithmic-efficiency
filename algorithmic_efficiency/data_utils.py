...
class PrefetchedWrapper:
    def __init__(self, dataloader, device, mean, std, start_epoch=0):
        ...
        self.device = device
        self.data_mean = torch.tensor([i / 255 for i in mean], device=device).view(
            1, 3, 1, 1
        )
        self.data_std = torch.tensor([i / 255 for i in std], device=device).view(
            1, 3, 1, 1
        )
    ...
    def prefetched_loader(self):
        ...
        for next_inputs, next_targets in self.dataloader:
            with torch.cuda.stream(stream):
                next_inputs = (
                    next_inputs.to(self.device, dtype=torch.float, non_blocking=True)
                    .sub(self.data_mean)
                    .div(self.data_std)
                )
