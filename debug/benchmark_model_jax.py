...
def main():
  print('=== JAX ResNet50 Model Benchmark ===')
  ...
  model = models.ResNet50(
    num_classes=NUM_CLASSES, act=nn.relu, dtype=jnp.float32
  )
  ...
  print(
    f'Model initialized. Param count: {sum(p.size for p in jax.tree.leaves(params)):,}'
  )
  ...
  def loss_fn(params, model_state, inputs, targets):
    ...
    per_example_loss = -jnp.sum(
      one_hot_targets * jax.nn.log_softmax(logits, axis=-1), axis=-1
    )
  ...
  inputs = jax.random.normal(
    data_rng, (BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, 3), dtype=jnp.float32
  )
  targets = jax.random.randint(
    jax.random.PRNGKey(43), (BATCH_SIZE,), 0, NUM_CLASSES
  )
  ...
  for i in range(NUM_WARMUP):
    start = time.perf_counter()
    loss, grads, new_model_state = train_step(
      params, model_state, inputs, targets
    )
