...
flags.DEFINE_boolean(
    'dump_training_batches',
    False,
    'Whether to dump batches to disk')
...
def train_once(
    ...
    dump_training_batches: Optional[bool] = None) -> Tuple[spec.Timing, Dict[str, Any]]:
  ...
  while train_state['is_time_remaining'] and \
    ...
    with profiler.profile('Data selection'):
      if dump_training_batches:
        batch = np.load("sharded_padded_batch_{}.npz".format(global_step), allow_pickle=True)
    ...
    if ((current_time - train_state['last_eval_time']) >=
      ...
      with profiler.profile('Evaluation'):
        try:
          ...
          train_state['validation_goal_reached'] = False
...
def score_submission_on_workload(workload: spec.Workload,
                                 ...
                                 log_dir: Optional[str] = None,
                                 dump_training_batches: Optional[bool] = None):
  ...
  if tuning_ruleset == 'external':
    ...
    with profiler.profile('Train'):
      score, _ = train_once(
          ...
          None, rng, profiler, max_global_steps, log_dir, dump_training_batches)
  return score
