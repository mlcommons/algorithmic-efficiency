...
def _check_ruleset_subdirs(submission_dir):
    ...
    if not ((EXTERNAL_TUNING in contents) or (SELF_TUNING in contents)):
        logging.info(f'CHECK FAILED: {submission_dir} does not contain ruleset subdir.')
        return False
    return True
        ...
        if parent_dir == SELF_TUNING or parent_dir == EXTERNAL_TUNING:
            for submission_dir in dirs:
                ...
                if SUBMISSION_MODULE not in contents:
                    logging.info(f'CHECK FAILED: {parent_dir} does not contain {SUBMISSION_MODULE}')
                    return False
    return True
        ...
        if parent_dir == EXTERNAL_TUNING:
            for submission_dir in dirs:
                ...
                if TUNING_SEARCH_SPACE_FILENAME not in contents:
                    logging.info(f'CHECK FAILED: {parent_dir} does not contain {TUNING_SEARCH_SPACE_FILENAME}')
                    return False
    return True


def check_submission_module_api()
...
def run_checks(submission_dir):
    ...
    contains_tuning_search_space_file = _check_tuning_search_space_file(submission_dir)

    if not (contains_ruleset_subdirs or contains_submission_module or contains_tuning_search_space_file):
